...
class TelegramBotApp(ctk.CTk):
        ...
        async def handler(event):
            ...
            if active_parse_method == "SniperPro":
                ...
                if current_signal_type_from_parser == SIGNAL_TYPE_INITIAL:
                    ...
                    # --- Pripraviť Akcie pre T2 (budú čakať na tickety) ---
                    func_config = config.get('functions', {})
                    if func_config.get('be_active'):
                        be_params = {'offset_pips': 1.0}
                        self._save_trade_action(action_type='SET_BE', trigger_event='ON_TICKET_PROFIT', params=be_params)
                    ...
                    if func_config.get('ts_active'):
                        ...
                        self._save_trade_action(action_type='ACTIVATE_TS', trigger_event='ON_TICKET_PROFIT', params=ts_params)
    ...
    def _save_trade_action(self, action_type: str, trigger_event: str,
                           target_ticket: int | None = None, trigger_ticket: int | None = None,
            ...
            try:
                c.execute("""
                    INSERT INTO trade_actions
                        (target_ticket, action_type, trigger_event, trigger_ticket, params_json, status)
                    VALUES (?, ?, ?, ?, ?, 'PENDING')
                """, (target_ticket, action_type, trigger_event, trigger_ticket, params_json_str))
                conn.commit()
                self._update_log(f"DB: Uložena akce '{action_type}' s triggerem '{trigger_event}'.", "INFO")
                return c.lastrowid
            except sqlite3.Error as e:
                self._update_log(f"DB Error: Nepodařilo se uložit akci {action_type}: {e}", "ERROR")
                return None
...
def report_trade():
    ...
    with db_lock, sqlite3.connect(DB_NAME) as conn:
        ...
        # If T1 is opened, find pending actions for T2 and link them
        if signal_row['trade_label'] == 'T1_AUTO':
            # First, get the ticket of the T2 trade
            c.execute("SELECT ticket FROM signals WHERE signal_group_id = ? AND trade_label = 'T2_AUTO'", (signal_row['signal_group_id'],))
            t2_row = c.fetchone()
            if t2_row and t2_row['ticket']:
                t2_ticket = t2_row['ticket']
                # Now, update the actions with both tickets
                c.execute("""
                    UPDATE trade_actions
                    SET trigger_ticket = ?, target_ticket = ?
                    WHERE trigger_ticket IS NULL AND target_ticket IS NULL
                """, (ticket, t2_ticket))
                if c.rowcount > 0:
                    log_msg += f" {c.rowcount} akcí pro T2 (ticket {t2_ticket}) bylo aktualizováno s trigger ticketem {ticket}."
